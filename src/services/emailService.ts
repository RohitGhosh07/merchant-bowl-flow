...
const EMAIL_FROM = 'noreply@rcgcbooking.in';
...
export const sendRegistrationEmail = async (formData: FormData) => {
  try {
    ...
    if (allEmails.size === 0) {
      console.warn('No email addresses provided for registration confirmation');
      return;
    }

    // Using Elastic Email's REST API directly
    const response = await fetch('https://api.elasticemail.com/v4/emails', {
      ...
      body: JSON.stringify({
        Recipients: [...allEmails].map(email => ({ Email: email })),
        Content: {
          From: EMAIL_FROM,
